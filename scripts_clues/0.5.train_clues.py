...
def verb_row(row: pd.DataFrame, template_t: str, tokenizer, verb_dict=None):
    """
    ...
    sentence = re.sub("<W1>", w1, template_t)
...
def run_model(yaml_dict: dict[Any, Any], config_name: str, model_name_inp: str):
    ...
    train_file = datadir + dataset_name + yaml_config["train_file"]
    test_file = datadir + dataset_name + yaml_config["test_file"]
    val_file = datadir + dataset_name + yaml_config["val_file"]
    ...
    # ORIGINAL CODE FROM LRC GITHUB.
    # SLIGHTLY MODIFIED FOR EFFICIENCY -THOMAS
    ...
    *****************************************"""
    ...
    # original way disabled
    # set a seeds to avoid equal trainings
    if False:
    ...
    args_train = TrainingArguments(
        ...
        evaluation_strategy="epoch" if val_file is not None else "no",
        save_strategy="epoch" if val_file is not None else "no",
        ...
        # fp16=True,
        logging_steps=10,
        load_best_model_at_end=True if val_file is not None else False,
        metric_for_best_model=metric_name,
        num_train_epochs=total_epochs,
        report_to="all",
        seed=seeds[0],
        save_total_limit=3,  # patience if val_file is not None else 0,
    )
    ...
    trainer = Trainer(
        ...
        eval_dataset=encoded_verb_val if val_file is not None else None,
        ...
        # callbacks = [EarlyStoppingCallback(early_stopping_patience=patience)] if val_file is not None and patience is not None else None
    )
