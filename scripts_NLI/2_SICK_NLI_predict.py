...
def get_preds(csvreader, dataset_path):    
    # to pipeline
    list_dict = []
    ...
    if preds_bool:
        ...
        result.to_csv(f"{output}/full/predicts_{part}_preds.tsv", sep="\t")
        result = pd.DataFrame(probs)
        result.to_csv(f"{output}/full/predicts_{part}_probs.tsv", sep="\t")
        ...
        res_group[["head", "tail", "preds", "probs"]].to_csv(f"{output}/inter/predictions_{part}.tsv", sep="\t")
