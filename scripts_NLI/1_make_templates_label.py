...
def to_template(SNLI_templates: list[tuple[str, str]], ID: str, scrapeID: str, w1: str, w2: str, label:str):
    for templatenum, (prem, hyp) in enumerate(SNLI_templates):
        ...
        yield f"{ID}\t{scrapeID}\t{templatenum}\t{prem_1}\t{hyp_2}\t{label}\n"
        yield f"{ID}\t{scrapeID}\t{templatenum}\t{prem_2}\t{hyp_1}\t{label}\n"
...
if __name__ == "__main__":
    ...
    insertfile.write(f"CombID\tSenID\ttemplatenum\tprem\thyp\tlabel\n")
    ...
    for csvline in csv.DictReader(datafile, delimiter="\t"):
        ...
        for template_line in to_template(SNLI_templates, csvline["ID"], csvline["scrapeID"],
                                         csvline["w1"], csvline["w2"], csvline["label"]):
            insertfile.write(template_line)

        # if int(csvline["scrapeID"]) > 99:
        #     break
