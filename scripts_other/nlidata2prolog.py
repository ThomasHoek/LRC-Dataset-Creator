...
def parse_arguments():
    ...
    parser.add_argument("file", metavar="PATH", help="File containing NLI data")
    parser.add_argument("output", metavar="PATH prefix", help="File for writing output")
    parser.add_argument("--fmt", required=True, choices=["spl", "sen.pl"], help="Format of the output file")
    parser.add_argument("--corpus", required=True, choices=["sick_semeval", "snlijson", "PPDB"], help="Corpus to be converted")
    parser.add_argument("--tokenize", default="nltk", choices=["raw", "nltk", "nltk_apo", "spacy", "native"],
                        help='How to tokenize the text. "native" works for snli.jsonl files which comes with parsed trees.')
    parser.add_argument("-v", "--verbose", dest="v", default=0, action="count", help="verbosity level of reporting")
...
def PPDB2nli(file_stream: list[str], out, fmt, tok="raw", v=0):
    """Convert stream into prolog or spl data and write it into file"""
    nli_dict = defaultdict(dict)
    i = 1
    for p in file_stream.readlines():
        (w1, w2, sen1, sen2, lab) = p.strip().split("\t")
        nli_dict[i] = {"pid": str(i), "p": sen1, "h": sen2, "g": lab.lower()}
        i += 1
    ...
    if fmt == "spl":
        ...
        cnt = nli_prolog(nli_dict, out, cen_labs=True)
        report("{} NLI sentences written".format(cnt), 1, v)
    else:
        raise RuntimeError("Unknown format /{}/".format(fmt))
...
def nli_prolog(nli_dict, out, cen_labs=True):
    ...
    count = 0
    cen2ynu = {"entailment": "yes", "contradiction": "no", "neutral": "unknown"}
    with open(out, "w") as f:
